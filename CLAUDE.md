...
- **Cross-package contract:** changing `@ymmv/shared` (esp. `types.ts` /
  ...
  (`MintRequest`, `MintResult`, `WhoamiResult`), which is **outside**
  `SCHEMA_VERSION`: the CLI refuses a mint or whoami reply missing a field,
  every `YMMV_TOKEN` command needs `GET /api/v1/auth/whoami` to exist, and
  every write command (`ymmv`, `set`, `unset`) needs the authed own-profile
  read `GET /api/v1/profile` plus `If-Match` handling on the POST (a 412 = the
